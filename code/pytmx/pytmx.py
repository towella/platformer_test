...
import logging
import os
import sys  # AT 17/07/2022 - import sys for modifying path in an executable to an absolute path from the base folder
import struct
import zlib
...
# AT 25/12/2022 - added parallax property, so it is cast to float in TiledElement > _cast_and_set_attributes_from_node_items
# would otherwise be default cast to str :(
        "parallaxx": float,
        "parallaxy": float,
...
class TiledElement:
    ...
    def _cast_and_set_attributes_from_node_items(self, items) -> None:
        # print(items)  # AT 25/12/2022 - Useful for determining properties of layers and objects from tiled
        # print(types)  # AT 25/12/2022 - Shows how properties are cast based on types dict (above)
        for key, value in items:
            casted_value = types[key](value)
            # AT 17/07/2022 - class attribute in tiled was not loaded into type and type can't be changed to class (reserved word)
            if key == 'class':
                key = 'type'
...
class TiledMap(TiledElement):
    ...
    def reload_images(self) -> None:
        ...
        for ts in self.tilesets:
            ...
            # AT 17/07/2022 - Modify path if using in an executable to be absolute from base folder rather than relative to tsx file
            try:
                # PyInstaller creates a temp folder and stores path in _MEIPASS
                base_path = sys._MEIPASS
                path = os.path.join(base_path, ts.source)
            except Exception:
    ...
    def parse_xml(self, node: ElementTree.Element) -> "TiledTileset":
        ...
        if source:
            if source[-4:].lower() == ".tsx":
                ...
                # AT 17/07/2022 - Modify path to absolute path from base folder if using in an executable. Otherwise remain relative to tmx
                ...
                #relative_path = os.path.join(dirname, source)
                try:
                    base_path = sys._MEIPASS
                    source = source[3:]
                    # PyInstaller creates a temp folder and stores path in _MEIPASS
...
class TiledTileLayer(TiledElement):
    ...
    def __init__(self, parent, node) -> None:
        ...
        # AT 25/12/2022 - Allows access of parallax values from tiled layer properties (see class TiledElement > _cast_and_set_attributes_from_node_items)
        # defaults to 1 (normal scrolling, i.e. scroll * parallax = scroll)
        self.parallaxx = 1
        self.parallaxy = 1
